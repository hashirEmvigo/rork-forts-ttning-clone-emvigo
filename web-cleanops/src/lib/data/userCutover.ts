/**
 * User (login) read-path source runtime (USER-2 / USER-4).
 *
 * The decision + telemetry layer for the Users cut-over — the Users analogue of
 * {@link import("./roleCutover")}. Users is a single GLOBAL company directory (an
 * in-memory array seeded once at app mount, holding both company logins and
 * platform super-admin logins). When the read path is enabled, the directory is
 * reconciled from Supabase via the validated {@link listFullUsersFromSupabase}
 * path while localStorage stays the backout copy.
 *
 * This module centralises:
 *   • source resolution — whether the directory should read from Supabase
 *     (granular USERS_SUPABASE_READ OR the USERS_SUPABASE_AUTHORITATIVE cut-over),
 *   • write resolution — whether login writes should be mirrored to Supabase
 *     (granular USERS_DUAL_WRITE OR the authoritative cut-over),
 ...
 *
 * It holds NO React state and performs NO I/O — the {@link useUserDirectorySource}
...
import {
  USERS_SUPABASE_READ,
  USERS_DUAL_WRITE,
  USERS_SUPABASE_AUTHORITATIVE,
} from "@/lib/featureFlags";

/** Which store currently backs the login directory the UI renders. */
export type UserReadSource = "supabase" | "localStorage";

/** Why a recorded read event served the localStorage seed instead of Supabase. */
export type UserReadFailureKind = "read" | "unsafe.empty" | "drift";

/** A single recorded fallback/failure/drift event during Supabase-read mode. */
export interface UserReadFailure {
  kind: UserReadFailureKind;
...
export interface UserCutoverState {
  ...
  recentFailures: UserReadFailure[];
}

const MAX_RECENT_FAILURES = 50;

function readEnabled(): boolean {
  return USERS_SUPABASE_READ || USERS_SUPABASE_AUTHORITATIVE;
}

const state: UserCutoverState = {
  supabaseRead: readEnabled(),
  supabaseWrite: USERS_DUAL_WRITE || USERS_SUPABASE_AUTHORITATIVE,
...
/**
 * Whether the login directory should READ from Supabase — true when the granular
 * read flag is on OR the authoritative cut-over is active (the latter implies the
 * read path).
 */
export function shouldReadUsersFromSupabase(): boolean {
  return readEnabled();
}

/**
 * Whether login writes should be MIRRORED to Supabase — true when the granular
 * dual-write flag is on OR the authoritative cut-over is active.
 */
export function shouldMirrorUserWrites(): boolean {
  return USERS_DUAL_WRITE || USERS_SUPABASE_AUTHORITATIVE;
}

/** Returns an immutable snapshot of the cumulative read-path telemetry. */
export function getUserCutoverState(): UserCutoverState {
  return {
    ...state,
    supabaseRead: readEnabled(),
    supabaseWrite: USERS_DUAL_WRITE || USERS_SUPABASE_AUTHORITATIVE,
...
export function resetUserCutoverState(): void {
...
function pushFailure(kind: UserReadFailureKind, ref: string, message: string): void {
 ...
 */
export function recordUserSupabaseRead(): void {
 ...
 */
export function recordUserReadFallback(ref: string, message: string): void {
  state.fallbacks += 1;
  state.localReads += 1;
  pushFailure("read", ref, message);
  if (import.meta.env.DEV === true) {
    // eslint-disable-next-line no-console
    console.warn(`[USER-2] Login directory fallback for ${ref}: ${message}`);
  }
}
 ...
 */
export function recordUserUnsafeEmpty(ref: string): void {
  ...
  pushFailure(
    "unsafe.empty",
    ref,
    "Supabase returned 0 logins while localStorage has data — kept local seed.",
  );
  if (import.meta.env.DEV === true) {
    // eslint-disable-next-line no-console
    console.warn(`[USER-2] Login directory unsafe empty for ${ref} — kept local seed.`);
  }
}
 ...
 */
export function recordUserShadowDrift(summary: string): void {
  state.shadowDrift += 1;
  state.lastMismatch = summary;
  pushFailure("drift", "shadow", summary);
  if (import.meta.env.DEV === true) {
    // eslint-disable-next-line no-console
    console.warn(`[USER-2] Login directory shadow drift: ${summary}`);
  }
}
