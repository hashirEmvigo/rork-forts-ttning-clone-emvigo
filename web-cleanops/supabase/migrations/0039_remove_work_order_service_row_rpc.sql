-- ============================================================================
-- CleanOps — Work Order service-row lifecycle
-- Remove one unprotected Work Order service row transactionally
-- ============================================================================
--
-- Removes a service row only when no protected operational/financial history is
-- attached. Removal is Supabase-authoritative and is represented as:
--   1. parent work_orders.data.serviceRows no longer contains the row
--   2. parent work_orders.service_row_count is decremented to match the aggregate
--   3. flat work_order_service_rows row is soft-deleted via deleted_at
--
-- Protected history currently enforced by durable Supabase tables:
--   * any active time_reports row for the service row
--   * execution Mission Log entries for the service row that have moved beyond
--     purely scheduled/not-started/cancelled planning states
--
-- This RPC intentionally does not touch booking_queue, schedule, occurrence
-- exceptions, mission log, time reporting, payroll, invoices, customer portal,
