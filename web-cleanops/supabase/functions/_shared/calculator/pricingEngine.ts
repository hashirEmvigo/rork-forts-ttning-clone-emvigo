/**
 * Price Calculator — PURE pricing engine (Deno port).
 *
 * ⚠️  FAITHFUL MIRROR of `src/lib/calculator/pricingEngine.ts` (the approved,
 *     tested engine). The two files are kept byte-for-byte equivalent in
 *     BEHAVIOUR by `src/lib/calculator/pricingEngineParity.test.ts`, which runs
 *     both through an identical input matrix and asserts deep-equal output. The
 *     ONLY intentional difference is the import specifier below (`./types.ts`
 *     with an explicit extension, required by Deno). Do not let this drift: any
 *     change to the approved engine must be mirrored here, and the parity test
 *     will fail until it is.
 *
 * Deterministic, side-effect-free price computation for the public calculator.
 * It performs NO I/O: the Edge Function loads the Supabase rows, maps them into
 * {@link PriceCalculationInput}, and calls this for the trusted server-side
 * price. Because pricing lives here — never in the DB or the UI — it is testable
 * in isolation and identical on every path.
 */
 ...
 * (Minutes per m²) for the home-cleaning size range that contains `sqm`. Mirror
 * of src/lib/calculator/pricingEngine.ts. First range (ascending fromSqm) whose
 * `[fromSqm, toSqm]` contains `sqm` wins (toSqm null = open-ended); no match → 0 %.
 */
...
export function calculateHomeCleaningPrice(
  ...
  // 1. Base per-visit time. Prefer the configurable SIZE BANDS (Slice 12J); fall
  //    back to the legacy linear model (base_hours + sqm × hours_per_sqm) when no
  //    band is configured, so old data + the parity matrix stay unaffected.
  // Slice 12Q: home per-sqm ADJUSTMENT ranges are PREFERRED over size bands.
...
/**
 * Maps a VALID result to the persisted snapshot shape (doc 04). Returns null for
 * invalid results so a priceless quote can never be persisted as if it had a
 * frozen total. (Reserved for the submit slice; not used by calculate yet.)
 */
